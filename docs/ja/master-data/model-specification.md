...
| id | string | - | スタッフID（一意識別子） |
| name | string | - | スタッフ名 |
| pin | string | - | 認証PIN（bcryptハッシュ化） |
| status | string | - | スタッフ状態（active/inactive） |
...
| item_details | array[string] | - | 追加情報リスト（文字列配列） |
...
| pos_x | integer | - | X座標位置 |
| pos_y | integer | - | Y座標位置 |
| size | ButtonSize | - | ボタンサイズ（SMALL/MEDIUM/LARGE等） |
| item_code | string | - | 商品コード |
| color_text | string | - | ボタンテキスト色 |
| image_url | string | - | ボタン画像URL |
| unit_price | float | - | 商品価格（動的取得、保存されない） |
| description | string | - | 商品説明（動的取得、保存されない） |

**注:** `unit_price` と `description` は商品マスターから動的に取得され、ドキュメントには保存されません。
...
| limit_amount | float | - | 上限金額（デフォルト: 0.0） |
...
|------------|------|----------|-------------|
| tax_code | string | - | 税コード |
| tax_type | string | - | 税タイプ（sales/vat等） |
| tax_name | string | - | 税名称 |
| rate | float | - | 税率（デフォルト: 0.0、例: 0.10 = 10%） |
| round_digit | integer | - | 丸め桁数（デフォルト: 0） |
| round_method | string | - | 丸め方法（up/down/nearest） |

**注:** 税マスターはテナント横断で共有されるため、`tenant_id`フィールドを持ちません。
